...
public class Process extends Element {
    ...
    private final ArrayList<Double> tNextList;

    public Process(String name, double delay, int channelsNumber) {
        super(name, delay);
        queue = 0;
        ...
        this.tNextList = new ArrayList<>();
        for (int i = 0; i < channelsNumber; i++) {
            this.tNextList.add(Double.MAX_VALUE);
        }
    }
    @Override
    public void inAct() {
        if (super.getWorkingProcessors() < this.channelsNumber) {
            ...
            this.tNextList.set(freeChannel, tCurrent + getDelay());
        } else {
    ...
    public void outAct() {
        ...
        this.tNextList.set(channelIdx, Double.MAX_VALUE);
        this.setWorkingProcessors(this.getWorkingProcessors() - 1);

        if (this.getQueue() > 0) {
            this.setQueue(this.getQueue() - 1);
            super.setWorkingProcessors(this.getWorkingProcessors() + 1);
            this.tNextList.set(channelIdx, tCurrent + getDelay());
        }
    ...
    public double getTnext() {
        ArrayList<Double> sortedList = new ArrayList<>(tNextList);
    ...
    public void printInfo() {
        super.printInfo();
        System.out.println(" відмови = " + this.getFailure() + "; }");
    }
    @Override
    ...
    private int getFinishedChannel() {
        for (int i = 0; i < this.channelsNumber; i++) {
            if(this.tNextList.get(i) == tCurrent) {
                return i;
            }
        }
    ...
    private int findFreeChannel() {
        for (int i = 0; i < channelsNumber; i++) {
            if(this.tNextList.get(i) == Double.MAX_VALUE) {
                return i;
            }
        }
